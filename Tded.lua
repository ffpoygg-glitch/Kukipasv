...
-- [เวอร์ชันอัปเดต: เพิ่มการเจาะค้นหาตัวแปร 9 d = และระบบเช็คผู้เล่นที่รันสคริปต์]
-- =====================================================
...
-- ==================== ระบบสื่อสารเช็คผู้เล่นที่รันสคริปต์ ====================
local ExecutedUsers = {} -- เก็บรายชื่อ [UserId] = {Player, DisplayName, Username, LastSeen}

-- ใช้ BindableEvent ภายในเครื่องเดียวกันในการ Sync
local HandshakeEventName = "Honkuki_Script_Handshake"
local HandshakeFolder = ReplicatedStorage:FindFirstChild("Honkuki_Comm")
if not HandshakeFolder then
    HandshakeFolder = Instance.new("Folder")
    HandshakeFolder.Name = "Honkuki_Comm"
    HandshakeFolder.Parent = ReplicatedStorage
end

local RemoteComm = HandshakeFolder:FindFirstChild("CommEvent")
if not RemoteComm then
    RemoteComm = Instance.new("UnreliableRemoteEvent")
    RemoteComm.Name = "CommEvent"
    RemoteComm.Parent = HandshakeFolder
end
...
function updateJunkViewerLive()
    ...
    if CurrentViewMode == 3 then
        ...
        -- ใส่ตัวเองลงในรายการเสมอ
        ExecutedUsers[LocalPlayer.UserId] = {
            Player = LocalPlayer,
        ...
        local count = 0
        local now = os.time()
        for uid, data in pairs(ExecutedUsers) do
            if data.Player and data.Player:IsDescendantOf(game) and (now - data.LastSeen < 12) then
                count = count + 1
                local profileLink = "https://www.roblox.com/users/" .. tostring(data.UserId) .. "/profile"
                outputText = outputText .. string.format("[%d] %s (@%s)\n - UserId: %d\n - ลิงก์โปรไฟล์: %s\n\n", count, data.DisplayName, data.Username, data.UserId, profileLink)
            else
                ExecutedUsers[uid] = nil
            end
        end

        if count == 0 then
            outputText = "❌ ไม่พบผู้เล่นอื่นที่รันสคริปต์นี้ในห้อง"
        else
...
-- Loop สื่อสาร Ping/Heartbeat ทั่วเซิร์ฟเวอร์
RemoteComm.OnClientEvent:Connect(function(senderPlayer, action)
    if senderPlayer and senderPlayer:IsA("Player") then
        if action == "Ping" or action == "Heartbeat" then
            ExecutedUsers[senderPlayer.UserId] = {
                Player = senderPlayer,
...
task.spawn(function()
    pcall(function()
        RemoteComm:FireServer("Ping")
    end)
    while true do
        task.wait(5)
        pcall(function()
            RemoteComm:FireServer("Heartbeat")
        end)
