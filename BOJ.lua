						...
						if event2 then
							...
							while true do
								if (0 == FlatIdent_6FA1) then
									args2 = {"ToolMusicText",tostring(musicId),"",[4]=true};
...
if IsAdmin then
	...
	while true do
		if (FlatIdent_6053C == 2) then
			aStroke = Instance.new("UIStroke", AdminCmdBtn);
...
StatusLabel.Text = (IsAdmin and "ระบบพร้อมเจาะข้อมูลผู้เล่น...") or "🛡️ สคริปต์ทำงานปกติ (รอรับคำสั่งจากแอดมิน)";
...
;p [IDเพลง] - สั่งให้ทุกคนเปิดเพลงนี้ทันที (ป้องกันเซ็นเซอร์ 100%)
	...
	if CurrentSelectedPlayer then
		...
		while true do
			...
			if (FlatIdent_72421 == 0) then
				StatusLabel.Text = "🎵 กำลังดึงเพลงเปิด...";
				targetPlayer = Players:FindFirstChild(CurrentSelectedPlayer.Name);
				FlatIdent_72421 = 1;
			end
...
local function ExecuteAdminCommand(cmd, targetArg, extraArg, senderUsername)
	if (senderUsername:lower() ~= AdminUsername:lower()) then
		return;
	end
		...
		elseif (cmd == "bring") then
			...
			while true do
				if (0 == FlatIdent_84C31) then
					adminPlayer = Players:FindFirstChild(senderUsername);
...
local function ProcessChatMessage(message, senderPlayer)
	...
	while true do
		...
		if (0 == FlatIdent_5202D) then
			if (not message or not senderPlayer) then
				return;
			end
			...
			if message.TextSource then
				...
				while true do
					if (FlatIdent_5077 == 0) then
						...
						if sender then
							ProcessChatMessage(message.Text, sender);
						end
						break;
					end
				end
