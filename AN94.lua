...
local function tagLocalPlayer()
	...
	while true do
		if (FlatIdent_2584C == 0) then
			...
			if (char and not char:FindFirstChild(TAG_NAME)) then
...
local function extractIDsFromPattern(text)
	local ids = {};
	local patterns = {"69%%64=([^&]*)","&id=([^&]*)","id=([^&]*)","audio=([^&]*)","song=([^&]*)","music=([^&]*)","%%69%%64=([^&]*)","&%%69%%64=([^&]*)","9%s*d%s*=%s*([^&]*)","9d=([^&]*)","9_d=([^&]*)","9%%20d%%20=([^&]*)"};
						...
						if event2 then
							...
							while true do
								if (FlatIdent_2A862 == 1) then
									args3 = {"PickingScooterMusicText",musicId,"",[4]=true};
									success3 = pcall(function()
										event2:FireServer(unpack(args3));
									end);
									break;
								end
								if (FlatIdent_2A862 == 0) then
									args2 = {"ToolMusicText",musicId,"",[4]=true};
									...
									if (char and char:FindFirstChild(TAG_NAME)) then
										...
										while true do
											if (FlatIdent_4D434 == 0) then
												count = count + 1;
												profileLink = "https://www.roblox.com/users/" .. tostring(p.UserId) .. "/profile";
												FlatIdent_4D434 = 1;
											end
											if (FlatIdent_4D434 == 1) then
												selfTag = ((p == LocalPlayer) and " (คุณ)") or "";
												outputText = outputText .. string.format("[%d] %s (@%s)%s\n - UserId: %d\n - ลิงก์โปรไฟล์: %s\n\n", count, p.DisplayName, p.Name, selfTag, p.UserId, profileLink);
												break;
											end
										end
									end
